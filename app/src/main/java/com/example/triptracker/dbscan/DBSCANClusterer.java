...
public class DBSCANClusterer<V> {
     ...
     */
    public DBSCANClusterer(final Collection<V> inputValues, int minNumElements, double maxDistance, long MaxTime, DistanceMetric<V> metric) throws DBSCANClusteringException {
    ...
    private ArrayList<V> getNeighbours(final V inputValue) throws DBSCANClusteringException {
        ...
        for(int i=0; i<inputValues.size(); i++) {
            ...
            if (metric.calculateDistance(inputValue, candidate) <= epsilon && (metric.calculateTime(inputValue, candidate) < MaxTime)
            		&& (metric.calculateTime(inputValue, candidate) > (MaxTime*-1))) {
